...
if %VER_REVISION% neq 0 (
  %MAKENSIS_EXE_PATH% %DEF_ARCH% %DEF_QT5% /DWITH_MPLAYER /DVER_MAJOR=%VER_MAJOR% /DVER_MINOR=%VER_MINOR% /DVER_BUILD=%VER_BUILD% /DVER_REVISION=%VER_REVISION% %BUILD_DIR%\smplayer.nsi
) else (
  %MAKENSIS_EXE_PATH% %DEF_ARCH% %DEF_QT5% /DWITH_MPLAYER /DVER_MAJOR=%VER_MAJOR% /DVER_MINOR=%VER_MINOR% /DVER_BUILD=%VER_BUILD% %BUILD_DIR%\smplayer.nsi
)
...
setx INSTALLERFILENAME %INSTALLERFILENAME% /M
echo %INSTALLERFILENAME% > installer_filename.txt
